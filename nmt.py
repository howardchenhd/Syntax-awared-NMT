...
layers = {'ff': ('param_init_fflayer', 'fflayer'),
          ...
          'tree_lstm': ('param_init_tlstm', 'tlstm_layer'),
          }
...
def prepare_treedata(seqs_x, seqs_y, seqs_tree, maxlen=None, n_words_src=30000,
    ...
    return x, x_mask, y, y_mask, mask_left, mask_right, tree_mask, write_mask
...
def param_init_lstm_cond(options, params, prefix='lstm_cond',
    ...
    params[_p(prefix, 'b_nl')] = numpy.zeros((4 * dim_nonlin,)).astype('float32')
...
def lstm_cond_layer(tparams, state_below, options, prefix='lstm',
    ...
    def _step_slice(m_, x_, h_, c_, ctx_, alpha_, pctx_, cc_,
                    U, Wc, W_comb_att, U_att, c_tt,
                    U_nl, b_nl):
        preact1 = tensor.dot(h_, U)
        preact1 += x_

        i = tensor.nnet.sigmoid(_slice(preact1, 0, dim))
        f = tensor.nnet.sigmoid(_slice(preact1, 1, dim))
        o = tensor.nnet.sigmoid(_slice(preact1, 2, dim))
        ...
        i = tensor.nnet.sigmoid(_slice(preact2, 0, dim))
        f = tensor.nnet.sigmoid(_slice(preact2, 1, dim))
        o = tensor.nnet.sigmoid(_slice(preact2, 2, dim))
        ...
        return h2, c2, ctx_, alpha.T  # pstate_, preact, preactx, r, u
    ...
    if one_step:
        rval = _step(*(seqs + [init_state, init_memory, None, None, pctx_, context] +
                       shared_vars))
    else:
        rval, updates = theano.scan(_step,
                                    sequences=seqs,
...
#Tree-LSTM
def param_init_tlstm(options, params, prefix='tlstm', nin=None, dim=None, dimctx=None):
    if dimctx is None:
    ...
    params[_p(prefix, 'b_f')] = numpy.zeros((2 * dimctx,)).astype('float32')
...
def tlstm_layer(tparams, mask_left, options, prefix='tlstm', mask=None, mask_right=None, write_mask=None, context=None, memory=None,
              **kwargs):
    ...
    assert context, 'Context must be provided'
    assert memory, 'Memory must be provided'
    ...
    # step function to be used by scan
    # arguments    | sequences (m_, x_, xx_)|outputs-info (h_)| non-seqs (U, Ux)
    def _step_slice(m_, left_, right_, write, ctx_, memery_, U_l, U_r, b, U_lf, U_rf, b_f, U_lc, U_rc, b_c):
        # hidden units
        #leftx = left_[:, :, None] * ctx_
        #rightx = right_[:, :, None] * ctx_
        #leftx_ = leftx.sum(0)                    # n_samples * dimctx
        #rightx_ = rightx.sum(0)
        
        leftx_ = (left_[:, :, None] * ctx_).sum(0)
        rightx_ = (right_[:, :, None] * ctx_).sum(0)

        # memory cells
        #leftm = left_[:, :, None] * memery_
        #rightm = right_[:, :, None] * memery_
        #leftm_ = leftm.sum(0)
        #rightm_ = rightm.sum(0)
        leftm_ = (left_[:, :, None] * memery_).sum(0)
        rightm_ = (left_[:, :, None] * memery_).sum(0)
        
        # io gates
        iogates = tensor.dot(leftx_, U_l) + tensor.dot(rightx_, U_r) + b

        i = tensor.nnet.sigmoid(_slice(iogates, 0, dim))
        o = tensor.nnet.sigmoid(_slice(iogates, 1, dim))

        # forget gates
        fgates = tensor.dot(leftx_, U_lf) + tensor.dot(rightx_, U_rf) + b_f

        f_l = tensor.nnet.sigmoid(_slice(fgates, 0, dim))
        f_r = tensor.nnet.sigmoid(_slice(fgates, 1, dim))

        # memory cell update
        c = tensor.tanh(tensor.dot(leftx_, U_lc) + tensor.dot(rightx_, U_rc) + b_c)
        c = i * c + f_l * leftm_ + f_r * rightm_
        ...
        memery_ = memery_ + write[:, None, None] * c
    ...
    memory = concatenate([memory, tensor.alloc(0., write_mask.shape[0], n_samples, dim)], axis=0)

    rval, updates = theano.scan(_step,
                                sequences=seqs,
                                outputs_info=[context, memory],
                                              # tensor.alloc(0., write_mask.shape[1], n_samples, dim)],
                                non_sequences=shared_vars,
                                name=_p(prefix, '_layers'),
                                n_steps=nsteps,
                                profile=profile,
...
def gru_cond_layer(tparams, state_below, options, prefix='gru',
                   mask=None, context=None, one_step=False,
                   init_memory=None, init_state=None, maxlen=None, 
                   context_mask=None,
                   **kwargs):
    ...
    def _step_slice(m_, x_, xx_, h_, ctx_, alpha_, pctx_, cc_,
        ...
        if maxlen:
            alpha_w = tensor.exp(alpha[:maxlen])
            alpha_t = tensor.tanh(alpha[maxlen:])
            if context_mask:
                alpha_w *= context_mask[:maxlen]
                alpha_t *= context_mask[maxlen:]
            alpha_w /= alpha_w.sum(0, keepdims=True)      # alignment weight
            alpha_t /= tensor.abs_(alpha_t).sum(0, keepdims=True)
            alpha = concatenate([alpha_w, alpha_t], axis=0)
        #    ctx_w = (cc_[:maxlen] * alpha_w[:, :, None]).sum(0)                  # [n_samples * dimctx]
        #    ctx_t = (cc_[maxlen:] * alpha_t[:, :, None]).sum(0)
        #    ctx_ = concatenate([ctx_w, ctx_t], axis=1)                           # [n_samples * 2dimctx]
        #    print 'maxlen!'
            # ctx_ = tensor.tanh(tensor.dot(ctx_w.sum(0), U_cw) + tensor.dot(ctx_t.sum(0), U_ct))
        else:
            alpha = tensor.exp(alpha)
            if context_mask:                       # context_mask = x_mask:[n_timestep * n_samples]
                alpha = alpha * context_mask       # [n_timestep * n_samples]
    ...
    shared_vars = [tparams[_p(prefix, 'U')],
                   ...
                   # tparams[_p(prefix, 'U_cw')],
                   # tparams[_p(prefix, 'U_ct')]]
...
def gru_cover_layer(tparams, state_below, options, prefix='gru',
    ...
    if one_step:
        rval = _step(*(seqs + [init_state, None, None, tensor.alloc(0., context.shape[0], n_samples, dim_cov),
...
def init_params(options):
    ...
    params = get_layer('ff')[0](options, params, prefix='ff_logit_ctx',
                                nin=ctxdim, nout=options['dim_word'],       #concatenate sequential and tree attention
                                ortho=False)
...
def build_model(tparams, options):
    ...
    mem = concatenate([proj[1], projr[1][::-1]], axis=proj[1].ndim-1)
    # added by chenhd: build tree-encoder
    #com_ctx
    proj_tree = get_layer(options['tree_encoder'])[1](tparams, mask_l, options,
                                                    prefix='encoder_tree',
                                                    mask=tree_mask, mask_right=mask_r, write_mask=wrt_mask, context=ctx, memory=mem)

    ctx_all = proj_tree[0][-1]
    mem_all = proj_tree[1][-1]   
    #ctx_all = com_ctx[0][-1]
    ...
    mem_mean = (mem_all * ctx_mask[:, :, None]).sum(0) / ctx_mask.sum(0)[:, None]  #n_samples * 2dim
    ...
    init_memory = get_layer('ff')[1](tparams, mem_mean, options,
    ...
    return trng, use_noise, x, x_mask, y, y_mask, mask_l, mask_r, tree_mask, wrt_mask, opt_ret, cost
...
def build_sampler(tparams, options, trng, use_noise):
    ...
    proj_tree = get_layer(options['tree_encoder'])[1](tparams, mask_l, options,
                                             prefix='encoder_tree',
                                             mask=None, mask_right=mask_r, write_mask=wrt_mask, context=ctx, memory=mem)
    ...
    outs = [init_state, init_memory, ctx_all]
    f_init = theano.function([x, mask_l, mask_r, wrt_mask], outs, name='f_init', profile=profile)
    print 'Done'
    ...
    inps = [y, ctx_all, init_state, init_memory]
...
def gen_sample(tparams, f_init, f_next, x, mask_l, mask_r, wrt_mask, options, trng=None, k=1, maxlen=30,
    ...
    ret = f_init(x, mask_l, mask_r, wrt_mask)
    next_state, next_memory, ctx0 = ret[0], ret[1], ret[2]
    next_w = -1 * numpy.ones((1,)).astype('int64')  # bos indicator

    for ii in xrange(maxlen):
        ctx = numpy.tile(ctx0, [live_k, 1])   # ctx0: [timesteps * dimctx]; ctx: [live_k * timesteps * dimctx]
...
def pred_probs(f_log_probs, prepare_treedata, options, iterator, verbose=True):
    ...
    for x, y, tree in iterator:
        ...
        x, x_mask, y, y_mask, mask_l, mask_r, tree_mask, wrt_mask = prepare_treedata(x, y, tree,
                                            n_words_src=options['n_words_src'],
                                            n_words=options['n_words'])

        pprobs = f_log_probs(x, x_mask, y, y_mask, mask_l, mask_r, tree_mask, wrt_mask)
        for pp in pprobs:
            probs.append(pp)
...
def train(dim_word=100,  # word vector dimensionality
          ...
          dim_cov=10,  # the number of coverage vector
          encoder='lstm',
          decoder='lstm_cond',
          tree_encoder='tree_lstm',
    ...
    trng, use_noise, \
        x, x_mask, y, y_mask, \
        mask_l, mask_r, tree_mask, wrt_mask, \
        opt_ret, \
        cost = \
        build_model(tparams, model_options)
    inps = [x, x_mask, y, y_mask, mask_l, mask_r, tree_mask, wrt_mask]
    ...
    for eidx in xrange(max_epochs):
        ...
        for x, y, tree in train:
            ...
            # filter by maxlen
            x, x_mask, y, y_mask, mask_l, mask_r, tree_mask, wrt_mask = prepare_treedata(x, y, tree, maxlen=maxlen,
            ...
            cost = f_grad_shared(x, x_mask, y, y_mask, mask_l, mask_r, tree_mask, wrt_mask)
